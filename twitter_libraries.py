...
def US_state_abrev(to_abreviation=True, vector_or_list=False, fips=False):
...
def query_twitter(
    searchTerms, certain_time_ago=None, number_of_tweets=None, location=None
):
    ...
    if certain_time_ago:
        ...
        query = "{} until:{} -filter:links -filter:retweets -filter:replies{}".format(
            searchTerms, certain_time_ago, q_loc
        )
    else:
        print("Query without until")
        query = "{} -filter:links -filter:retweets -filter:replies{}".format(
            searchTerms, q_loc
        )
...
def create_df(name, random_food, days_ago=None, number_of_tweets=None, location=None):
    ...
    final_data = pd.DataFrame(
        data=None, columns=["date", "location", "username", "search", "tweet"]
    )
    ...
    if type(random_food) == list:
        ...
        for food in random_food:
            ...
            iter_data = query_twitter(
                food, certain_time_ago, number_of_tweets, location
            )
        ...
        # print('A STRING')
        iter_data = query_twitter(
            random_food, certain_time_ago, number_of_tweets, location
        )
        print(len(final_data))
...
def get_metrics(path):
    ...
    get_ipython().run_cell_magic("R", "", """library(ldatuning)""")
    get_ipython().run_cell_magic(
        "R",
        "",
        """\n
    ...
    get_ipython().run_cell_magic(
        ...
        """\n
                       ...
                       ap_lda <- LDA(dtm, k = {}, method= "Gibbs",
                                     control = list(seed = 576,alpha=2))""".format(
            number_of_topics
        ),
    )
